...
| **Server-Side Auth (`llm-proxy/server.js:200-240`)** | `REAL` | Node.js `crypto.scryptSync` + `timingSafeEqual` with bearer session tokens. | Shipped. Needs removal of client fallback. |
| **Client-Side Auth (`src/core/saas/auth.ts:1305`)** | `MOCK / INSECURE` | Unsalted SHA-256 browser hash + `localStorage` storage (`tacf_auth_store_v2`). | **DEPRECATE & REMOVE** (Consolidate to server). |
